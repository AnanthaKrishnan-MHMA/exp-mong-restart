...
app.get('/countries', async (req, res) => {
    ...
    if (continent) {
        const countries = await Countries.find({ continent: continent })
        res.render('countries', { countries, continent })
    } else {
        const countries = await Countries.find({})
        res.render('countries', { countries, continent: 'all' })
    }
})
...
app.delete('/countries/:id', async (req, res) => {
    const { id } = req.params
    console.log(req.body)
    const foundCountry = await Countries.findByIdAndDelete(id)
    res.redirect(`/countries`)
})
app.listen(port, () => {
